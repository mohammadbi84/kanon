        ...
        <div class="widget-content widget-content-area br-6">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">حرفه</li>
                </ol>
            </nav>
            <br>
            ...
            <div class="widget-header">
                <div class="row">
                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                        <h2>لیست سال‌های موجود</h2>
                        <a href="{{ route('tuition_herfe.create') }}" class="btn btn-success">درج شهریه</a>
                        <br>
                        <br>
                    </div>
                </div>
                        ...
                        @foreach ($years as $key => $year)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a
                                        onclick="window.location.href='{{ route('tuition_herfe.showYear', $year->year) }}'">{{ $year->year }}</a>
                                </td>
                                <td>{{ $year->title }}</td>
                                <td>{{ \Morilog\Jalali\Jalalian::fromDateTime($year->start_date)->format('Y/m/d') }}</td>
                                <td>{{ \Morilog\Jalali\Jalalian::fromDateTime($year->end_date)->format('Y/m/d') }}</td>
                                <td>{{ $year->cityName }}</td>
                                <td>{{ \Morilog\Jalali\Jalalian::fromDateTime($year->created_at)->format('Y/m/d') }}</td>

                                <td>
                                    <a onclick="window.location.href='{{ route('tuition_herfe.showYear', $year->year) }}'"
    ...
    <script>
        $(document).ready(function() {
            $('#updateButton').click(function() {
                ...
                $.ajax({
                    url: "{{ route('tuition_herfe.updateAll') }}",
                    type: "POST",
                    data: formData,
                    success: function(response) {
                        // نمایش پیام موفقیت یا خطا
                        $('#statusMessage').html('<div class="alert alert-success">' + response
                            .message + '</div>');
                    },
                    error: function(xhr) {
                        // نمایش پیام خطا
                        $('#statusMessage').html(
                            '<div class="alert alert-danger">مشکلی پیش آمده است، لطفا دوباره تلاش کنید.</div>'
                            );
                    }
                });
            });
